...
const Mybooks = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:8000/books?limit=10");
        setBooks(res.data);
      } catch (error) {
  ...
  return (
    <div className="w-full mt-[40px]  px-[100px] relative  ">
      ...
      <div className='justify-center flex mt-11'>
                    <Link to='/books' className='w-[120px] h-[40px] py-2 px-2 rounded-3xl text-center
                    font-semibold opacity-80 hover:opacity-100 hover:scale-105
